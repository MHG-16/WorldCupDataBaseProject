...
echo "$($PSQL "SELECT DISTINCT(name) FROM games AS g1 RIGHT JOIN teams ON g1.winner_id = teams.team_id LEFT JOIN games AS g2 ON teams.team_id = g2.opponent_id WHERE (g1.year=2014 AND g1.round='Eighth-Final') OR (g2.year=2014 AND g2.round='Eighth-Final')")" 
